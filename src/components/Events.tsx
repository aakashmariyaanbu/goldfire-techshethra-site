...
const Events = () => {
  ...
  const filteredEvents = activeCategory === 'all' ? eventsList : eventsList.filter(event => event.category === activeCategory);
  return <section id="events" className="section bg-gradient-to-b from-[#0e0e10] to-[#121215] text-white">
      <div className="container mx-auto">
        <div className="mb-16 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-8">
            {eventCategories.map(category => <button key={category.id} onClick={() => setActiveCategory(category.id)} className={`rounded-full px-4 py-2 text-sm font-medium transition-all ${activeCategory === category.id ? 'bg-gold text-black' : 'bg-white/10 text-white hover:bg-white/20'}`}>
                {category.name}
              </button>)}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredEvents.map(event => <div key={event.id} className="bg-black/50 backdrop-blur-sm rounded-xl overflow-hidden card-hover">
